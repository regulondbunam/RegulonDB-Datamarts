...
class Regulator(BiologicalBase):
    ...
    def to_dict(self):
        ...
        if self.regulator.regulation_type[0] == "Transcription-Factor-Binding":
            regulator = {
                ...
                "type": self.regulator.regulation_type[0]
            }
        else:
            abbreviated_name = None
            if self.regulator.regulation_type[0] == "RNA-Mediated-Translation-Regulation":
    ...
    def products(self, regulator):
        self._products = []
        if regulator.regulation_type[0] == "transcriptionFactor":
            for product_id in regulator.products_ids:
                prod = multigenomic_api.products.find_by_id(product_id)
                prod_dict = {
                    "_id": prod.id,
