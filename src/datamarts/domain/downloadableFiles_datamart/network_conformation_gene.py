...
class ConformationGene:
    ...
    def objects(self):
        ...
        for ri_object in ri_objects:
            # print(ri_object.id)
            ri_row = ConformationGene.ConformationGeneDatamart(ri_object)
            yield ri_row
        del ri_objects

    class ConformationGeneDatamart:
...
def get_all_rows(rdb_version, citation):
    trans_factors = ConformationGene()
    tfs_content = [
        "1)regulatorId\t2)regulatorName\t3)RegulatorGeneName\t4)conformationName\t5)regulatedId\t6)regulatedName\t7)function\t8)confidenceLevel"]
    ...
    tfs_doc = {
        ...
        "fileName": "NetworkRegulatorConfGene",
        ...
        "columnsDetails": "# Columns:\n"
                          ...
                          "# (4) conformationName.  Name of the active conformation of the regulator.\n"
                          "# (5) regulatedId. Gene ID regulated by the Regulator (regulated Gene)\n"
                          "# (6) regulatedName. Gene regulated by the Regulator (regulated Gene)\n"
                          "# (7) function. Regulatory Function of the Regulator on the regulated Gene (+ activator, - repressor, -+ dual, ? unknown)\n"
                          "# (8) confidenceLevel. RI confidence level based on its evidence (Values: Confirmed[C], Strong[S], Weak[W], Unknown[?])",
        "content": " \n".join(tfs_content),
        "rdbVersion": rdb_version,
        "description": "Regulatory Network Interactions between of the active conformation of the regulator and their regulated genes.",
